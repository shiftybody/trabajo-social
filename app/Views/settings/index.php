  ...
  <div class="content">
    <div class="config-container">
      <nav class="config-nav">
        <?php if (\App\Core\Auth::canAny(['settings.levels.view', 'settings.rules.view'])): ?>
          <div class="config-nav-group">
            <h3>General</h3>
            <hr>
          </div>
        <?php endif; ?>
        <?php if (\App\Core\Auth::canAny(['settings.criteria.view'])): ?>
          <div class="config-nav-group">
            <h3>Criterios</h3>
          </div>
        <?php endif; ?>
...
<script>
   ...
   * - se decidio no usar un archivo separado debido a que era 
   * - necesario realizar validaciones de permisos
   *
  ...
  class ConfigManager {
    constructor() {
      <?php if (\App\Core\Auth::can('settings.levels.view')): ?>
    ...
    /**
     * Renderiza toda la navegación de criterios dinámicamente
     *
     */
  ...
  `;
    ...
    /**
     * Carga criterios por subcategoría
     */
    async loadCriteriaBySubcategory(subcategoryId) {
      ...
      <div class="config-content-header">
        ...
        <?php if (\App\Core\Auth::can('settings.criteria.create')): ?>
        ...
        <table id="criteria-table" class="hover nowrap cell-borders" style="width: 100%;">
          <thead>
            <tr>
              ...
              <?php if (\App\Core\Auth::can('settings.criteria.edit')): ?>
              <th class="dt-head-center">ESTADO</th>
              <?php endif; ?>
              <?php if (\App\Core\Auth::canAny(['settings.criteria.edit', 'settings.criteria.delete'])): ?>
    ...
    /**
     * Inicializa DataTable para criterios
     */
    async initializeCriteriaTable(subcategoryId) {
      ...
      // Agregar columna de estado solo si tiene permisos
      <?php if (\App\Core\Auth::can('settings.criteria.edit')): ?>
        columns.push({
          data: "estado",
          className: "dt-body-center",
      ...
      <label class="toggle-switch">
        <input type="checkbox" ${data == 1 ? "checked" : ""} 
               onchange="configManager.toggleCriteriaStatus(${row.id}, this.checked)">
        <span class="toggle-slider"></span>
      </label>`
        });
      <?php endif; ?>

      // Agregar columna de acciones solo si tiene permisos
      <?php if (\App\Core\Auth::canAny(['settings.criteria.edit', 'settings.criteria.delete'])): ?>
        columns.push({
          data: null,
          className: "dt-body-center",
        ...
        <button type="button" class="editar" onclick="configManager.editCriteria(${row.id})" title="Editar Criterio">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-pencil">
            ...
            <?php if (\App\Core\Auth::can('settings.criteria.delete')): ?>
              actions += `
        <button type="button" class="remover" onclick="configManager.deleteCriteria(${row.id})" title="Eliminar Criterio">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-trash">
            ...
            return actions || '<span class="no-actions">Sin acciones disponibles</span>';
          }
        });
      <?php endif; ?>
      ...
      this.currentTable = new DataTable("#criteria-table", {
        ...
        columns: columns, // Usar el array dinámico
        paging: false,
        info: false,
        order: [
          [1, "asc"]
        ...
        language: {
          ...
          zeroRecords: "No se encontraron criterios que coincidan con la búsqueda",
        },
      });
    }
    ...
    updateActiveNavLink(section) {
      ...
      // Si no hay sección especificada o no hay links, salir
      ...
      // Si hay una sección especificada, buscarla
      if (section) {
        targetLink = Array.from(allNavLinks).find(link =>
          link.getAttribute("href") === `#${section}`
        );
      }

      // Si no se encuentra o no hay sección, usar el primer link
      if (!targetLink) {
        targetLink = allNavLinks[0];

        // Actualizar la sección actual al primer link
        const firstSection = targetLink.getAttribute("href")?.replace("#", "");
        if (firstSection) {
          this.currentSection = firstSection;
          localStorage.setItem('configActiveSection', firstSection);

          // Opcional: cargar la sección automáticamente
          this.loadSection(firstSection);
        }
      }

      // Activar el link encontrado
    ...
    <div class="config-content-header">
      ...
      <?php if (\App\Core\Auth::can('settings.levels.create')): ?>
      ...
      <table id="levels-table" class="hover nowrap cell-borders" style="width: 100%;">
        <thead>
          <tr>
            ...
            <?php if (\App\Core\Auth::can('settings.levels.edit')): ?>
            <th class="dt-head-center">ESTADO</th>
            <?php endif; ?>
            <?php if (\App\Core\Auth::canAny(['settings.levels.edit', 'settings.levels.delete'])): ?>
            <th class="dt-head-center">ACCIONES</th>
            <?php endif; ?>
          </tr>
        </thead>
  ...
  `;
    ...
    async initializeLevelsTable() {
      ...
      <label class="toggle-switch">
        <input type="checkbox" ${data == 1 ? "checked" : ""} 
               onchange="configManager.toggleLevelStatus(${row.id}, this.checked)">
        <span class="toggle-slider"></span>
      </label>`
        });
      <?php endif; ?>
      ...
      <?php if (\App\Core\Auth::canAny(['settings.levels.edit', 'settings.levels.delete'])): ?>
        ...
        <button type="button" class="editar" onclick="configManager.editLevel(${row.id})" title="Editar Nivel">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-pencil">
            ...
            <?php if (\App\Core\Auth::can('settings.levels.delete')): ?>
              actions += `
    ...
    <div class="config-content-header">
      ...
      <?php if (\App\Core\Auth::can('settings.rules.create')): ?>
      ...
      <table id="rules-table" class="hover nowrap cell-borders" style="width: 100%;">
        <thead>
          <tr>
            ...
            <th class="dt-head-center">PERIODICIDAD</th>
            <th class="dt-head-center">MONTO</th>
            <?php if (\App\Core\Auth::can('settings.rules.edit')): ?>
            <th class="dt-head-center">ESTADO</th>
            <?php endif; ?>
            <?php if (\App\Core\Auth::canAny(['settings.rules.edit', 'settings.rules.delete'])): ?>
            <th class="dt-head-center">ACCIONES</th>
            <?php endif; ?>
          </tr>
        </thead>
    ...
    async initializeRulesTable() {
      ...
      <?php if (\App\Core\Auth::can('settings.rules.edit')): ?>
        columns.push({
          ...
          render: (data, type, row) =>
            `<label class="toggle-switch">
        <input type="checkbox" ${data == 1 ? "checked" : ""} 
               onchange="configManager.toggleRuleStatus(${row.id}, this.checked)">
        <span class="toggle-slider"></span>
      </label>`
        });
      <?php endif; ?>
      ...
      <?php if (\App\Core\Auth::canAny(['settings.rules.edit', 'settings.rules.delete'])): ?>
        ...
        <button type="button" class="editar" onclick="configManager.editRule(${row.id})" title="Editar Regla">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-pencil">
            ...
            <?php if (\App\Core\Auth::can('settings.rules.delete')): ?>
              actions += `
      ...
      this.currentTable = new DataTable("#rules-table", {
        ...
        language: {
          emptyTable: "El nivel no ha sido seleccionado o no tiene reglas configuradas",
          processing: '<div class="table-spinner"></div>Procesando...',
