  ...
  constructor() {
    this.rules = {
      ...
      pattern: (value, rule) => {
        ...
        return !regex.test(value) ? rule.message || "Formato no válido" : null;
      },
      ...
      email: (value, rule) => {
        ...
        return !emailRegex.test(value)
          ? rule.message || "Correo electrónico no válido"
          : null;
      },
      ...
      matches: (value, rule, formData) => {
        ...
        return value !== otherValue
          ? rule.message || "Los campos no coinciden"
          : null;
      },
      ...
      fileSize: (file, rule) => {
        ...
        return file.size > rule.value
          ? rule.message ||
              `Archivo muy grande. Máximo ${rule.value / 1024 / 1024}MB`
          : null;
      },
      ...
      fileType: (file, rule) => {
        ...
        return !rule.value.includes(file.type)
          ? rule.message || "Tipo de archivo no válido"
          : null;
      },
      ...
      min: (value, rule) => {
        ...
        return value < rule.value
          ? rule.message || `El valor mínimo es ${rule.value}`
          : null;
      },
      max: (value, rule) => {
        if (typeof value !== "number") return null;
        return value > rule.value
          ? rule.message || `El valor máximo es ${rule.value}`
          : null;
      },
    };
  }
